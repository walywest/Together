...
class FitgirlExtractorApp:
    ...
    def get_browser_path(self):
        ...
        import sys
        is_wsl = sys.platform != 'win32'

        if is_wsl:
            import glob
            # Scan Windows user profiles to find LocalAppData equivalents
            local_app_data_dirs = [
                p.replace('/AppData', '/AppData/Local')
                for p in glob.glob('/mnt/c/Users/*/AppData')
                if os.path.basename(os.path.dirname(p)) not in ('Public', 'Default', 'Default User', 'All Users')
            ]
            browser_paths = []
            ...
            for lad in local_app_data_dirs:
                browser_paths += [
                    f"{lad}/Google/Chrome/Application/chrome.exe",
                    f"{lad}/BraveSoftware/Brave-Browser/Application/brave.exe",
                    f"{lad}/Microsoft/Edge/Application/msedge.exe",
                ]
